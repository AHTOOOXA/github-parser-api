...
async def top100(order_by: Optional[str] = None, db: Database = Depends(get_db)):
    if order_by:
        db.get_cur().execute(f"SELECT * FROM repositories ORDER BY {order_by}")
    else:
...
async def activity(owner: str, repo: str, db: Database = Depends(get_db)):
    db.get_cur().execute("SELECT repo_id FROM repositories WHERE owner = %s AND repo = %s", (owner, repo))
